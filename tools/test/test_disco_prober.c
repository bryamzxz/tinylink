...
static void test_basic_roundtrip(void) {
    ...
    fails += ok("basic/match-once", disco_prober_match_and_remove(txid, 2000));
    fails += ok("basic/match-twice-fails",
                !disco_prober_match_and_remove(txid, 2000));
}
...
static void test_multiple_txids(void) {
    ...
    /* Out-of-order matches all work. */
    fails += ok("multi/match-b", disco_prober_match_and_remove(b, 1500));
    fails += ok("multi/match-c", disco_prober_match_and_remove(c, 1500));
    fails += ok("multi/match-a", disco_prober_match_and_remove(a, 1500));
    /* All consumed. */
    fails += ok("multi/a-gone", !disco_prober_match_and_remove(a, 1500));
}
...
static void test_timeout(void) {
    ...
    /* Record at t=0 µs. Match at t = timeout_ms + 1 ms → expired. */
    int64_t now0 = 0;
    ...
    fails += ok("timeout/stale-match-fails",
                !disco_prober_match_and_remove(txid, now1));
    ...
    fails += ok("timeout/refresh-then-match",
                disco_prober_match_and_remove(txid, now1 + 100));
}
...
static void test_lru_eviction(void) {
    ...
    fails += ok("lru/table-full-all-match-before-eviction",
                disco_prober_match_and_remove(txids[0],
    ...
    fails += ok("lru/newer-matches",
                disco_prober_match_and_remove(newer,
    ...
    fails += ok("lru/second-oldest-survives",
                disco_prober_match_and_remove(txids[1],
                    base + DISCO_PROBER_TABLE_SIZE * 100 + 1));
}
...
static void test_unknown_txid(void) {
    ...
    fails += ok("unknown/no-match",
                !disco_prober_match_and_remove(unknown, 1500));
    /* Known still matches afterwards. */
    fails += ok("unknown/known-still-matches",
                disco_prober_match_and_remove(known, 1500));
}
...
static void test_null_safety(void) {
    ...
    fails += ok("null/match-returns-false",
                !disco_prober_match_and_remove(NULL, 1000));
}
